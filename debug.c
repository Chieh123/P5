...
int checkRoot(struct dinode inode) {
 fprintf(stderr, "===========in checRoot========type  (%d).====\n", inode.type);
    if(inode.type != T_DIR) {
        fprintf(stderr, "ERROR: root directory does not exist. 1\n");
        return 1;
    }
    ...
    if(parent == NULL || self == NULL) {
        fprintf(stderr, "ERROR: root directory does not exist.2\n");
        return 1;
    }
    if(parent->inum != self->inum) {
        fprintf(stderr, "ERROR: root directory does not exist. 3\n");
        return 1;
    }
    return 0;
}
...
printf("\n\ninode%d size is %d cnt = %d\n", index_n, inode.size, cnt);
    if(cnt == 0) {
...
printf("i = %d j = %d addr = %d name = %s\n", i, j, temp[j].inum, temp[j].name);
    ...
    if(self->inum != index_n) {
        fprintf(stderr, "ERROR: directory not properly formatted. inode# = ");
        return 1;
//        return 0;
    }
fprintf(stderr, "inode# = %d", index_n);
...
//        fprintf(stderr, "inode.addrs[%d] = %d BLOCKS = %d\n", i, inode.addrs[i], BLOCKS);
        if(cheackValid(inode.addrs[i]) && inode.addrs[i] > BLOCKS) {
...
printf("block_index = %d", block_index);
    int j = 0;
    for(i = 0; i < INODES; i++){
